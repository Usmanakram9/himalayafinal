...
const subProductStore = (set) => ({
  ...
  addSubProduct: async (newSubProduct) => {
    try {
      set({ isLoading: true, error: null });
      const response = await axios.post('http://localhost:8000/api/subproducts', newSubProduct);
  ...
  deleteSubProduct: async (subProductId) => {
    try {
      set({ isLoading: true, error: null });
      await axios.delete(`http://localhost:8000/api/subproducts/${subProductId}`);
      set((state)=>({
        data: state.data.filter((subProduct)=> subProduct._id !== subProductId),
        isLoading: false,
      }));
    } catch (error) {
  ...
  updateSubProduct: async (subProductId, updatedSubProduct) => {
    try {
      set({ isLoading: true, error: null});
      const response = await axios.put(`http://localhost:8000/api/subproducts/${subProductId}`, updatedSubProduct);
      set((state)=> ({
        data: state.data.map((product)=> (product._id === subProductId ? response.data : product)),
        isLoading: false,
      }))
    } catch (error) {
